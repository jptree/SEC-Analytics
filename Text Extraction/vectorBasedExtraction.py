...
def create_close_model(wv, df_extracted):
    ...
    for m, file_name in enumerate(files):
        ...
        if text == '-9':
            continue
        file_text = open(f'{PATH}/{file_name}').read()
        close_index = file_text.index(text)
        controls_indices = [m.start() for m in re.finditer('controls', file_text, re.IGNORECASE)]
        quantitative_indices = [m.start() for m in re.finditer('quantitative', file_text, re.IGNORECASE)]
        part_indices = [m.start() for m in re.finditer('part', file_text, re.IGNORECASE)]
        item_indices = [m.start() for m in re.finditer('item', file_text, re.IGNORECASE)]
        interested_locations = [0, 100, 200, 500, 700, 900, 1100, 1500, close_index]
        interested_locations += controls_indices
        interested_locations += item_indices
        interested_locations += quantitative_indices
        interested_locations += part_indices

        for p in interested_locations:
            surrounding_text = file_text[p - 500: p + 500]
            features = get_close_features(surrounding_text, file_text, wv, p)

            X.append(features)
            y.append(1 if p == close_index else 0)
